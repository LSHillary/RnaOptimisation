...
rule PostFilteringMultiQC:
    input:
        CheckFilteringFastQC = expand("Checks/FilteredFastQC_{sample}_{run}.done", sample = rna_viromes, run = RnaRuns)
    output:
...
rule PostQCCleanup:
    input:
        CheckDedup = expand("Checks/2.5_Dedup_{sample}_{run}.done", sample = rna_viromes, run = RnaRuns)
    output:
