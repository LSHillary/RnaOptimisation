...
rule all:
    input:
        CheckDnaFastQC = expand("Checks/1.1-RawFastQC_{sample}_{run}.done", sample=rna_viromes, run=RnaRuns),
        CheckDnaMultiQC = expand("Checks/1.2-MultiQC_{run}.done", run = RnaRuns),
        CheckPostCoAssemblyCleanup = "Checks/1-PostPreprocessingCleanup.done"

# Rule 1.1 - Run FastQC on merged runs
rule RawFastQC:
    input:
        ForRaw = lambda wildcards: f"0-raw/{wildcards.run}/{wildcards.sample}_{get_RnaRunID(wildcards.run)}_R1_001.fastq.gz",
        RevRaw = lambda wildcards: f"0-raw/{wildcards.run}/{wildcards.sample}_{get_RnaRunID(wildcards.run)}_R2_001.fastq.gz",
    output:
...
rule RawMultiQC:
    input:
        CheckRawFastQC = expand("Checks/1.1-RawFastQC_{sample}_{run}.done", sample = rna_viromes, run = RnaRuns)
    output:
...
rule PostPreProcessingCleanup:
    input:
        CheckMultiQC = expand("Checks/1.2-MultiQC_{run}.done", run = RnaRuns)
    output:
        CheckPostCoAssemblyCleanup = "Checks/1-PostPreprocessingCleanup.done"
    params:
        tag = "1-PreprocessingCleanup",
        local_folder = "1-Preprocessing/",
        remote_folder = "FARM_archive/RnaOptimisation/Phase2/RnaVirome/1-Preprocessing/"
    shell:'''
